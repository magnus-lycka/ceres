...
class CommonMishap1Severe(ChoiceBase):
    ...
    label: str = 'Severely injured (Injury table result 2: roll 1D, choose STR/DEX/END to reduce by that amount)'
    ...
    def handle(self, projection: CharacterProjection, event_id: int, pending_idx: int) -> int:
        projection.pending_inputs.append(
            PendingChoices(
                ...
                choices=[
                    CommonMishap1Severe(stay_in_career=self.stay_in_career),
                    CommonMishap1DoubleRoll(stay_in_career=self.stay_in_career),
                ],
            )
        )
        return pending_idx + 1
