"""Typst proof-of-concept: Dragon Free Trader on A4.

Run with:
    uv run --with typst python examples/pdf_typst_dragon.py

Output: tests/ships/generated_output/pdf/dragon_typst.pdf
"""
...
def spec_table_rows() -> str:
    ...
    for section, rows in groupby(main_rows, key=lambda r: r.section):
        rows = list(rows)
        n = len(rows)
        # 0pt guard column: breakable:false keeps entire section together
...
// Description
#v(5mm)
#text(size: 9pt, weight: "bold", fill: accent)[Description & Operating Notes]
#v(2mm)

The Dragon-class is a needle-hulled combat courier optimised for high-threat
insertion and extraction missions. Its streamlined profile and improved stealth
coating allow it to operate in contested space with minimal sensor signature,
while the crystaliron armour and particle barbette give it genuine offensive
and defensive capability against light opposition. Radiation shielding and a
full biosphere make extended independent operations practical without requiring
external support.

The vessel is crewed by a tight specialist team of eight. The captain doubles
as mission commander; the two pilots stand watch-and-watch on long transits.
A dedicated sensor operator manages the extended array suite, which includes
countermeasures and enhanced signal processing for both detection and jamming.
The gunner manages the particle barbette and missile bay in concert; the small
missile storage capacity of 480 rounds requires careful logistics planning on
operations longer than a few weeks.

Maintenance notes: the improved stealth coating requires specialist inspection
every 8 weeks of operation. The point defence battery Type II-L fire-control
linkage is known to drift under sustained high-power cycling and should be
recalibrated after any engagement in which more than 40 rounds are expended.
Repair drones are rated for hull and structural work only; drive and power
plant maintenance requires the workshop and at least one qualified engineer.
...
#grid(
  ...
  table(
    columns: (0pt, 16mm, 1fr, 14mm, 14mm),
...
out_path = out_dir / 'dragon_typst.pdf'
