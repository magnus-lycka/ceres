...
class TestPsionicCommunity:
    ...
    def test_failed_entry_returns_to_career_choice(self):
        ...
        assert projection.summary.current_precareer_term is None
...
def test_precareer_in_progress_is_typed_object():
    ...
    assert isinstance(term.precareer, PreCareerData)
...
def test_precareer_completed_is_typed_object():
    ...
    projection = replay(1, events)
    completed = next((t for t in projection.summary.terms if isinstance(t, PreCareerTerm) and t.completed), None)
    assert completed is not None
    assert isinstance(completed.precareer, PreCareerData)
