...
def _events() -> list:
    return [
        ...
        # Term 1: qualify Scout Courier, survive, event 5 (benefit_dm), advancement fails, reenlist
        ...
        # Term 2: service skill roll, survive, event 5 (benefit_dm), advancement fails, muster out
        SkillTableEvent(id=9, fulfills='8.0', table='service_skills', roll=1),  # Pilot → small_craft 0→1
        SurviveEvent(id=10, fulfills='9.0', roll=7),
        TermEventEvent(id=11, fulfills='10.0', roll=5),  # benefit_dm +1 to muster out roll
        AdvancementEvent(id=12, fulfills='11.0', roll=5),  # fails
        ReenlistEvent(id=13, fulfills='12.0', reenlist=False),
        # Muster out: 2 rolls (term_count=2, rank=0)
        MusterOutEvent(id=14, fulfills='13.0', table='benefits', roll=6),  # scout_ship
        MusterOutEvent(id=15, fulfills='13.1', table='cash', roll=1),  # Cr20,000
    ]
