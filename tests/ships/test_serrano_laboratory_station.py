...
def build_serrano_laboratory_station() -> ship.Ship:
    return ship.Ship(
        ...
        systems=SystemsSection(
            probe_drones=AdvancedProbeDrones(count=15),
            laboratories=[Laboratory()] * 24,
...
def test_serrano_laboratory_station_matches_reference_sheet():
    ...
    assert station.systems.probe_drones.tons == pytest.approx(3.0)
    assert station.systems.probe_drones.cost == pytest.approx(2_400_000.0)
