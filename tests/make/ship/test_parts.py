...
def test_part_rejects_ship_below_tl():
    ...
    assert part.notes.errors == ['Requires TL15, ship is TL14']
...
def test_customisable_part_build_notes_appends_customisation_note():
    ...
    assert 'High Technology: Energy Efficient, Size Reduction × 2' in part.notes.infos
...
def test_customisable_part_rejects_disallowed_customisation_on_bind():
    ...
    part.bind(DummyShip())
    assert 'Modification not allowed for CustomPart: Orbital Range' in part.notes.errors
...
def test_customisable_part_without_customisation_uses_part_tl():
    ...
    assert part.notes.errors == ['Requires TL12, ship is TL11']
...
def test_customisable_part_applies_customisation_tl_delta(customisation, ship_tl, expected):
    ...
    assert expected in part.notes.errors
...
def test_prototype_warns_when_ship_tl_is_high_enough_without_it():
    ...
    assert 'Prototype not required: ship TL12 exceeds required TL11' in part.notes.warnings
...
def test_early_prototype_warns_when_prototype_would_suffice():
    ...
    part.bind(DummyShip(tl=11))
    assert 'Early Prototype not required: ship TL11 exceeds required TL10' in part.notes.warnings
