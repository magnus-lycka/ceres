...
def test_large_ship_reduces_engineering_and_other_scaling_roles():
    my_ship = ship.Ship(
        ...
        power=PowerSection(plant=FusionPlantTL12(output=490)),
        command=CommandSection(bridge=Bridge()),
        computer=ComputerSection(hardware=Computer5()),
    )
...
def test_commercial_ship_gets_gunner_for_barbette():
    my_ship = ship.Ship(
        ...
        command=CommandSection(bridge=Bridge()),
        computer=ComputerSection(hardware=Computer5()),
        weapons=WeaponsSection(barbettes=[PulseLaserBarbette()]),
    )
...
def test_military_ship_gets_gunners_for_bays():
    my_ship = ship.Ship(
        ...
        hull=hull.Hull(configuration=hull.standard_hull),
        drives=DriveSection(m_drive=MDrive2()),
        power=PowerSection(plant=FusionPlantTL12(output=100)),
        command=CommandSection(bridge=Bridge()),
        computer=ComputerSection(hardware=Computer5()),
...
def test_steward_requirement_uses_skill_levels_for_large_middle_passenger_manifest():
    my_ship = ship.Ship(
        ...
        hull=hull.Hull(configuration=hull.streamlined_hull),
        drives=DriveSection(m_drive=MDrive1(), j_drive=JDrive1()),
        power=PowerSection(plant=FusionPlantTL12(output=40)),
        command=CommandSection(bridge=Bridge()),
        computer=ComputerSection(hardware=Computer5()),
