...
def _projection_with_term(characteristics: dict[Chars, int] | None = None) -> CharacterProjection:
    ...
    return CharacterProjection(
        ...
        summary=CharacterSummary(
            ...
            terms=[CareerTerm(career=SCOUT, assignment=courier)],
        ),
    )
...
class TestChoiceBenefit:
    def _make(self) -> ChoiceBenefit:
        return ChoiceBenefit(
            options=[
                CharacteristicIncrease(char=Chars.SOC, amount=1),
                YACHT,
            ]
    ...
    def test_display_label_joins_options_with_or(self):
        choice = self._make()
        assert choice.display_label == 'SOC +1 or Yacht'

    def test_exceptional_when_any_option_is_exceptional(self):
        choice = self._make()
        assert choice.exceptional

    def test_not_exceptional_when_no_option_is_exceptional(self):
