...
# ── PendingPsionicTalentLevelChoice event_from_form and input_specs ───────────


class TestPendingPsionicTalentLevelChoice:
    def _pending(self) -> PendingPsionicTalentLevelChoice:
        return PendingPsionicTalentLevelChoice(pending_id=(1, 0), instruction='Level up talent', level=2)

    def _talent_json(self) -> str:
        ta = TypeAdapter(PsionicTalentSkills)
        return ta.dump_json(Telepathy()).decode()

    def test_event_from_form_creates_talent_level_event(self) -> None:
        event = self._pending().event_from_form({'talent': self._talent_json()})
        assert isinstance(event.handler, PsionicTalentLevelHandler)
        assert isinstance(event.handler.talent, Telepathy)

    def test_input_specs_returns_empty_when_psionics_is_none(self) -> None:
        projection = CharacterProjection(character_id=1, summary=_summary())
        assert self._pending().input_specs(projection) == []


# ── PendingPsionicInstituteTraining event_from_form and input_specs ───────────


class TestPendingPsionicInstituteTrainingFormHandling:
    def _pending(self) -> PendingPsionicInstituteTraining:
        return PendingPsionicInstituteTraining(
            pending_id=(1, 0),
            instruction='Train',
            remaining_talents=[Telepathy(), Telekinesis()],
        )
    ...
    def test_event_from_form_finish_creates_finish_handler(self) -> None:
        event = self._pending().event_from_form({'talent': 'finish'})
        assert isinstance(event.handler, FinishPsionicInstituteTrainingHandler)

    def test_event_from_form_talent_choice_creates_training_handler(self) -> None:
        event = self._pending().event_from_form({'talent': self._talent_json(), 'roll': '8'})
    ...
    def test_input_specs_returns_talent_select_and_roll_entry(self) -> None:
        projection = _psionic_projection(psi=9)
        specs = self._pending().input_specs(projection)
        assert len(specs) == 2
        assert any(isinstance(s, Select) for s in specs)
        assert any(isinstance(s, NumberEntry) for s in specs)


# ── PsiStrengthTestHandler narrative branches ─────────────────────────────────
...
class TestPsiStrengthTestHandlerNarrative:
    ...
    def test_psi_zero_logs_no_psionic_strength_remaining(self) -> None:
        ...
        projection.summary.terms = [CareerTerm(career=SCOUT, assignment=courier)] * 3
        handler = PsiStrengthTestHandler(roll=2)

        handler.apply(projection, Event(handler=handler))

        assert any('no Psionic Strength remaining' in n for n in projection.summary.narrative)


# ── PendingLifeEventPsionicsRoll form handling ────────────────────────────────


class TestPendingLifeEventPsionicsRoll:
    def _pending(self) -> PendingLifeEventPsionicsRoll:
        return PendingLifeEventPsionicsRoll(pending_id='life_psi', instruction='Roll PSI test')

    def test_event_from_form_creates_psi_strength_test_event(self) -> None:
        event = self._pending().event_from_form({'roll': '8'})
        assert isinstance(event.handler, PsiStrengthTestHandler)
        assert event.handler.roll == 8

    def test_input_specs_returns_number_entry(self) -> None:
        specs = self._pending().input_specs(CharacterProjection(character_id=1, summary=_summary()))
        assert len(specs) == 1
        assert isinstance(specs[0], NumberEntry)


# ── PendingInitialPsiTest event_from_form ────────────────────────────────────


class TestPendingInitialPsiTestEventFromForm:
    def test_yes_creates_accepted_handler(self) -> None:
        pending = PendingInitialPsiTest(pending_id=(1, 0))
        event = pending.event_from_form({'test': 'yes'})
        assert isinstance(event.handler, InitialPsiTestAcceptedHandler)

    def test_no_creates_declined_handler(self) -> None:
        pending = PendingInitialPsiTest(pending_id=(1, 0))
        event = pending.event_from_form({'test': 'no'})
        assert isinstance(event.handler, InitialPsiTestDeclinedHandler)


# ── PendingInitialPsiStrengthRoll form handling ───────────────────────────────


class TestPendingInitialPsiStrengthRoll:
    def _pending(self) -> PendingInitialPsiStrengthRoll:
        return PendingInitialPsiStrengthRoll(pending_id=(2, 0), instruction='Roll for PSI')

    def test_event_from_form_creates_initial_psi_test_event(self) -> None:
        event = self._pending().event_from_form({'roll': '9'})
        assert isinstance(event.handler, InitialPsiTestHandler)
...
def test_queue_initial_psi_test_or_career_choice_queues_psi_test_when_available() -> None:
    non_imperial = MOCK_WORLD.model_copy(update={'allegiance': 'NaHu'})
    projection = CharacterProjection(
        character_id=1,
        summary=CharacterSummary(name='Psi', sophont=VILANI, homeworld=non_imperial),
    )

    queue_initial_psi_test_or_career_choice(projection, event_id=1)

    assert any(isinstance(p, PendingInitialPsiTest) for p in projection.pending_inputs)


# ── Psionics.raise_talent_to: untrained talent ────────────────────────────────
...
def test_raise_talent_to_raises_for_untrained_talent() -> None:
    ...
    with pytest.raises(ValueError, match='Cannot improve untrained'):
        psionics.raise_talent_to(Telepathy, level=1)
